...
`v0.15` is the second major version after we open sourced Bottender. In this version, we introduces a lot of helpful, community requested features based on [Messaging APIs v0.7](https://github.com/Yoctol/messaging-apis/releases/tag/v0.7.0).
...
- [new] implement Batch Mode to sends multiple requests in one batch (up to 50 messages):
...
- [breaking] Now context methods throws error when `ok` is `false` in Telegram:
